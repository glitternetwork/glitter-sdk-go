general comments
1. comment with full setences, even though it seems a little bit redundant, it makes them format well when extracted to 
documentation.
2. Let's capilize error strings.
...
BlockChainInfo gets blockchain info(metadata) by height\.
...
BlockSearch searches blocks between BeginBlock and EndBlock events (inclusive?)\.
...
Health returns health of the node\. It will return empty result \(200 OK\) on success\ and no response \- in case of an error\.
...
NetInfo gets informatin of the network\.
...
New creates a new Glitter client\.
...
Chain provides chain api to search transactions and block info\.
...
Cluster provides cluster api to manager validator or nodes\. 
...
DB provides database api to create schema, store and access documents\.
...
ListValidators lists information of the validators\.
...
RemoveValidator removes the given validator from Glitter cluster\.
...
UpdateValidator updates validator in Glitter cluster. which will be updated? power?
...
CreateSchema creates the schema formatted in JSON\. add comments that indexed field should be specified.
...
GetDocs returns a list of documents with the provided IDs in the schema\.
...
ListSchema lists all Glitter schemas\.
...
PutDoc puts the document to Gliter and returns the transcation id\.
...
Search searches from Glitter database with given condition\.
...
MarshalJSON returns m as the JSON encoding of m\. what is m? should this as d? make sure names are unique for one type and consistant.
...
please try to add some comments to all functions and below.
