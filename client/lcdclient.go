...
func (lcd *LCDClient) EditDatasetRequest(ctx context.Context, options CreateTxOptions, datasetName string, workStatus chaindepindextype.ServiceStatus, hosts string, manageAddresses string, meta string, description string) (*sdk.TxResponse, error) {
...
func (lcd *LCDClient) RenewalDataset(ctx context.Context, options CreateTxOptions, datasetName string, duration int64) (*sdk.TxResponse, error) {
	_msg := chaindepindextype.NewRenewalDatasetRequest(lcd.GetAddress(), datasetName, duration)
	options.Msgs = []msg.Msg{_msg}
	return lcd.SignAndBroadcastTX(ctx, options)
}

func (lcd *LCDClient) Pledge(ctx context.Context, options CreateTxOptions, datasetName string, amount sdk.Int) (*sdk.TxResponse, error) {
	_msg := chaindepconsumertype.NewPledgeRequest(lcd.GetAddress(), datasetName, amount)
	options.Msgs = []msg.Msg{_msg}
	return lcd.SignAndBroadcastTX(ctx, options)
}

func (lcd *LCDClient) ReleasePledge(ctx context.Context, options CreateTxOptions, datasetName string, amount sdk.Int) (*sdk.TxResponse, error) {
